...
class Plotter:
    ...
    def draw_label(self, image: numpy.ndarray, x1, y1, text, colour, thickness):
        '''
        Given a img, starting x,y cords, text and colour, create a filled in box of specified colour
        and write the text
        '''
        #font_scale = 0.5 # 
        font_scale = max(0.5,0.0000005*max(image.shape))
        p = 5 #padding
        text_size, _ = cv2.getTextSize(text, self.font, font_scale, thickness)
        cv2.rectangle(image, (x1-p, y1-p), (x1+text_size[0]+p, y1-text_size[1]-(2*p)), colour, -1)
